...
class MainWindow(QMainWindow):
    ...
    @QtCore.pyqtSlot()
    def get_screen_region_and_open_save_file_dialog1(self) -> None:
    ...
    def get_screen_region_and_open_save_file_dialog(self, display=False):
        # Save a copy of the selected region X, Y, Width and Height.
        self.region_x_pos = self.x()
        self.region_y_pos = self.y()
        self.region_width = self.width()
        self.region_height = self.height()
        print(self.region_x_pos, self.region_y_pos, self.region_width, self.region_height)
        # Open the Save File Dialog for the user to choose the location and the name
        # for the video to save.
        region = (self.region_x_pos, self.region_y_pos, self.region_width, self.region_height)
        # Hide both windows.
        # self.hide()
        # self.button_window.hide()
        if region is None:
        ...
        video_file_name, _ = QFileDialog.getSaveFileName(self, "Save File", "", "AVI Files (*.avi)")
        time.sleep(1)

        w, h = self.region_width, self.region_height
        video_writer = cv2.VideoWriter(video_file_name, codec, 30, (w, h))
...
class ButtonWindow(QWidget):
    def __init__(self) -> None:
        ...
        button_stylesheet = "QPushButton {" \
                            "color: white;" \
                            "font: 10pt;" \
                            "background-color: rgb(6, 104, 249);" \
                            "border: 2px inset rgba(151, 222, 247, 255);" \
                            "border-radius: 5px;" \
                            "}"
